...
import ListingAIReport from "../components/ai/ListingAIReport";
...
function AdminMarketplace() {
  ...
  async function sendProductDecision(productId, decision, evidence = {}) {
    setBusy(productId);
  ...
  async function requestProductDecision(productId, decision) {
    const promptText = decision === "changes_requested"
      ? "What exactly should the seller change?"
      : "Why is this product being rejected?";
    const note = window.prompt(promptText);
    if (note === null) return;
    if (!note.trim()) {
      toast.error("Enter a clear reason for the seller.");
      return;
    }
    await sendProductDecision(productId, decision, { note });
  }

  async function approveProduct(event, productId) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    await sendProductDecision(productId, "approved", {
      inspectionMethod: form.get("inspectionMethod"),
      actualImagesVerified: form.get("actualImagesVerified") === "on",
      flexProofVerified: form.get("flexProofVerified") === "on",
      note: form.get("note"),
    });
  }

  async function runAIListingReview(productId) {
    setBusy(`ai-${productId}`);

    try {
      const data = await apiRequest(`/api/ai/admin/listings/${productId}/review`, {
        method: "POST",
        token,
        timeout: 60_000,
      });
      setProducts((current) => current.map((product) =>
        product._id === productId
          ? { ...product, aiReview: data.aiReview }
          : product
      ));
      toast.success(data.message);
    } catch (requestError) {
              ...
              {products.length === 0 ? (
                ...
                <div className="mt-7 grid gap-5">
                  {products.map((product) => {
                    ...
                    return (
                      <article key={product._id} className="overflow-hidden rounded-2xl border border-slate-200">
                        <div className="grid gap-5 p-5 lg:grid-cols-[140px_1fr_auto] lg:items-center">
                          <img src={product.imageUrl} alt={product.name} className="h-32 w-full rounded-xl object-cover lg:w-32" />
                          <div>
                            <div className="flex flex-wrap items-center gap-2"><h3 className="text-xl font-black">{product.name}</h3><span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-bold text-amber-700">Pending verification</span>{product.videoUrl && <span className="inline-flex items-center gap-1 rounded-full bg-violet-100 px-2.5 py-1 text-xs font-bold text-violet-700"><Film size={13} />FlexProof submitted</span>}</div>
                            <p className="mt-2 text-sm font-bold text-orange-600">{product.store?.name} · {product.category}</p>
                            <p className="mt-3 line-clamp-2 leading-6 text-slate-600">{product.description}</p>
                            <p className="mt-3 text-sm text-slate-500">{money(product.price)} · {product.stock} in stock · Seller: {product.seller?.firstName} {product.seller?.lastName}</p>
                          </div>
                          <div className="flex flex-wrap gap-2 lg:w-48 lg:flex-col">
                            <button type="button" onClick={() => setExpandedProductId(expanded ? "" : product._id)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-3 font-bold text-white"><Eye size={17} />{expanded ? "Close details" : "View product"}</button>
                            <button type="button" disabled={busy === `ai-${product._id}`} onClick={() => runAIListingReview(product._id)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-violet-200 px-4 py-3 font-bold text-violet-700 hover:bg-violet-50 disabled:opacity-50"><Sparkles size={17} />{busy === `ai-${product._id}` ? "Checking…" : "AI quality check"}</button>
                            <button type="button" disabled={busy === product._id} onClick={() => requestProductDecision(product._id, "changes_requested")} className="inline-flex items-center justify-center gap-2 rounded-xl border border-violet-200 px-4 py-3 font-bold text-violet-700 hover:bg-violet-50 disabled:opacity-50"><Pencil size={17} />Request changes</button>
                            <button type="button" disabled={busy === product._id} onClick={() => requestProductDecision(product._id, "rejected")} className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-200 px-4 py-3 font-bold text-red-700 hover:bg-red-50 disabled:opacity-50"><XCircle size={17} />Reject</button>
                          </div>
                        </div>

                        {expanded && (
                          <div className="border-t border-slate-200 bg-slate-50 p-5 sm:p-7">
                            <ListingAIReport report={product.aiReview} className="mb-6" />

                            <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
                              <div>
                                <h4 className="text-lg font-black">Seller-submitted trust evidence</h4>
                                <dl className="mt-4 grid gap-3 sm:grid-cols-2">
                                  <div className="rounded-xl bg-white p-4"><dt className="text-xs font-bold uppercase text-slate-500">Condition</dt><dd className="mt-1 font-bold capitalize">{product.condition?.replaceAll("_", " ") || "Not supplied"}</dd></div>
                                  <div className="rounded-xl bg-white p-4"><dt className="text-xs font-bold uppercase text-slate-500">Warranty</dt><dd className="mt-1 font-bold">{product.warranty || "Not supplied"}</dd></div>
                                  <div className="rounded-xl bg-white p-4"><dt className="text-xs font-bold uppercase text-slate-500">Returns</dt><dd className="mt-1 font-bold">{product.returnWindowDays > 0 ? `${product.returnWindowDays}-day return` : "Not return eligible"}</dd></div>
                                  <div className="rounded-xl bg-white p-4"><dt className="text-xs font-bold uppercase text-slate-500">Fulfilment</dt><dd className="mt-1 font-bold capitalize">{product.fulfillmentType === "flexhub" ? "FlexHub" : "Seller"}</dd></div>
                                  <div className="rounded-xl bg-white p-4 sm:col-span-2"><dt className="text-xs font-bold uppercase text-slate-500">Preparation time</dt><dd className="mt-1 font-bold">Ships in {product.dispatchTimeDays ?? 1} day(s)</dd></div>
                                </dl>
                                <div className="mt-5 rounded-xl bg-white p-5"><p className="text-xs font-bold uppercase text-slate-500">Full description</p><p className="mt-3 whitespace-pre-line leading-7 text-slate-700">{product.description}</p></div>
                                {product.videoUrl && <div className="mt-5 overflow-hidden rounded-2xl bg-slate-950 p-4"><div className="mb-3 flex items-center gap-2 font-bold text-white"><Film className="text-orange-400" />Submitted product video</div><video src={product.videoUrl} controls className="max-h-[440px] w-full rounded-xl" /></div>}
                              </div>

                              <form onSubmit={(event) => approveProduct(event, product._id)} className="h-fit rounded-2xl border border-emerald-200 bg-white p-5 shadow-sm">
                                <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 text-emerald-600" /><div><h4 className="font-black">FlexHub verification</h4><p className="mt-1 text-sm leading-6 text-slate-500">Record only checks actually completed.</p></div></div>
                                <label className="mt-5 grid gap-2 text-sm font-bold">Verification method
                                  <select name="inspectionMethod" defaultValue="digital" className="rounded-xl border border-slate-300 bg-white px-3 py-3 font-normal">
                                    <option value="digital">Digitally verified</option>
                                    <option value="physical">Physically inspected</option>
                                    <option value="not_inspected">Listing review only</option>
                                  </select>
                                </label>
                                <label className="mt-4 flex items-start gap-3 rounded-xl bg-slate-50 p-4 text-sm"><input type="checkbox" name="actualImagesVerified" className="mt-1 h-4 w-4 accent-emerald-600" /><span><strong className="block">Actual images confirmed</strong><span className="mt-1 block text-slate-500">The images reasonably represent the item offered.</span></span></label>
                                {product.videoUrl && <label className="mt-3 flex items-start gap-3 rounded-xl bg-slate-50 p-4 text-sm"><input type="checkbox" name="flexProofVerified" className="mt-1 h-4 w-4 accent-emerald-600" /><span><strong className="block">FlexProof video confirmed</strong><span className="mt-1 block text-slate-500">The submitted video matches this listing.</span></span></label>}
                                <label className="mt-4 grid gap-2 text-sm font-bold">Internal approval note <span className="font-normal text-slate-400">(optional)</span><textarea name="note" rows="3" maxLength="300" className="rounded-xl border border-slate-300 px-3 py-3 font-normal" placeholder="What was checked?" /></label>
                                <button type="submit" disabled={busy === product._id} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 font-black text-white hover:bg-emerald-700 disabled:opacity-50"><CheckCircle2 size={18} />Approve & Flex Verify</button>
                              </form>
                            </div>
                          </div>
                        )}
