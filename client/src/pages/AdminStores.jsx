...
function AdminStores() {
  ...
  const [verificationStoreId, setVerificationStoreId] = useState("");
  ...
  useEffect(() => {
    const timer = window.setTimeout(loadStores, 0);
  ...
  async function saveVerification(event, store) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    const checkNames = ["email", "phone", "identity", "address", "business", "physicalInspection"];
    const body = Object.fromEntries(checkNames.map((name) => [name, form.get(name) === "on"]));
    setBusyId(store._id);

    try {
      const data = await apiRequest(`/api/stores/admin/${store._id}/verification`, {
        method: "PATCH",
        token,
        body: JSON.stringify(body),
      });
      toast.success(data.message);
      setVerificationStoreId("");
      await loadStores();
    } catch (error) {
      toast.error(error.message);
    } finally {
            ...
            <div className="mt-10 grid gap-6">
              {visibleStores.map((store) => (
                <article key={store._id} className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
                  <div className="flex flex-col gap-7 xl:flex-row xl:items-start xl:justify-between">
                    ...
                    <div className="flex shrink-0 flex-wrap gap-3 xl:max-w-sm xl:justify-end">
                      ...
                      {store.status === "approved" && (
                        <>
                          <button type="button" onClick={() => setVerificationStoreId(verificationStoreId === store._id ? "" : store._id)} className="inline-flex items-center gap-2 rounded-xl border border-emerald-300 px-4 py-3 font-bold text-emerald-700 hover:bg-emerald-50"><ShieldCheck size={18} />Verification</button>
                          <button
                            type="button"
                            disabled={busyId === store._id}
                  ...
                  {verificationStoreId === store._id && store.status === "approved" && (
                    <form onSubmit={(event) => saveVerification(event, store)} className="mt-7 rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5">
                      <div className="flex items-start gap-3"><ShieldCheck className="text-emerald-700" /><div><h3 className="font-black text-emerald-950">Seller verification evidence</h3><p className="mt-1 text-sm text-emerald-800">Tick only checks FlexHub has genuinely completed. Identity and address checks unlock the Verified Seller badge.</p></div></div>
                      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        {[
                          ["email", "Email confirmed"],
                          ["phone", "Phone confirmed"],
                          ["identity", "Identity verified"],
                          ["address", "Business address checked"],
                          ["business", "Business evidence checked"],
                          ["physicalInspection", "Physical inspection completed"],
                        ].map(([name, label]) => (
                          <label key={name} className="flex items-center gap-3 rounded-xl border border-emerald-100 bg-white p-4 text-sm font-bold">
                            <input type="checkbox" name={name} defaultChecked={Boolean(store.verificationChecks?.[name])} className="h-4 w-4 accent-emerald-600" />{label}
                          </label>
                        ))}
                      </div>
                      <button disabled={busyId === store._id} className="mt-5 rounded-xl bg-emerald-700 px-5 py-3 font-black text-white disabled:opacity-50">Save verification evidence</button>
                    </form>
                  )}
                </article>
              ))}
