...
function ProductImagePicker({ file, onChange, disabled = false }) {
  ...
  return (
    <div>
      <label className="mb-2 block text-sm font-semibold text-slate-300" htmlFor={inputId}>
        Main product image
      </label>
      ...
      {preview ? (
        ...
        <label htmlFor={inputId} className={`flex min-h-64 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-700 bg-slate-900 p-8 text-center transition ${disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:border-orange-500 hover:bg-slate-900/70"}`}>
          ...
          <span className="mt-5 font-bold text-white">Choose a clear product photo</span>
          <span className="mt-2 text-sm leading-6 text-slate-400">JPG, PNG or WebP · maximum 5 MB</span>
          <span className="mt-5 inline-flex items-center gap-2 rounded-xl bg-slate-800 px-4 py-2.5 text-sm font-bold text-slate-200"><UploadCloud size={17} /> Browse files</span>
        </label>
      )}
