    ...
    <header className="sticky top-0 z-50 border-b border-slate-800 bg-slate-950/95 shadow-lg backdrop-blur-xl">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex h-20 items-center gap-4">
          <Link to="/" className="shrink-0" aria-label="Nigerian marketplace home">
          ...
          <nav className="ml-auto hidden items-center gap-6 text-sm font-semibold lg:flex" aria-label="Main navigation">
            {navItems.map(([label, path]) => (
              <NavLink key={path} to={path} className={navClass}>{label}</NavLink>
            ))}
          </nav>
          ...
          <div className="ml-auto flex shrink-0 items-center gap-3 lg:ml-2">
            ...
            {user ? (
              <>
                <Link to="/profile" aria-label="Your account" className="flex h-9 w-9 items-center justify-center rounded-full bg-orange-500 font-bold text-white">
                  {user.firstName?.charAt(0) || user.name?.charAt(0) || "U"}
                </Link>
                <button type="button" onClick={logout} aria-label="Log out" className="hidden rounded-lg p-2 text-slate-400 hover:bg-slate-800 hover:text-red-400 sm:block">
                  <LogOut size={20} />
                </button>
              </>
            ) : (
            ...
            <button type="button" onClick={() => setMenuOpen((value) => !value)} aria-expanded={menuOpen} aria-label="Toggle navigation" className="rounded-lg p-2 text-white hover:bg-slate-800 lg:hidden">
        ...
        {menuOpen && (
          <div className="border-t border-slate-800 py-4 lg:hidden">
            ...
            <nav className="mt-3 grid gap-1" aria-label="Mobile navigation">
              {navItems.map(([label, path]) => (
                <NavLink key={path} to={path} onClick={() => setMenuOpen(false)} className={({ isActive }) => `rounded-xl px-4 py-3 font-semibold ${isActive ? "bg-orange-500/10 text-orange-400" : "text-slate-300 hover:bg-slate-900"}`}>{label}</NavLink>
              ))}
              {user ? (
                <button type="button" onClick={logout} className="flex items-center gap-2 rounded-xl px-4 py-3 text-left font-semibold text-red-400 hover:bg-slate-900"><LogOut size={18} />Log out</button>
              ) : (
                <Link to="/login" className="rounded-xl px-4 py-3 font-semibold text-slate-300 hover:bg-slate-900">Log in</Link>
              )}
            </nav>
            <Link to="/sell" className="mt-3 flex items-center justify-center gap-2 rounded-xl border border-orange-500 px-4 py-3 font-bold text-orange-400"><Store size={18} />Open a mini-store</Link>
          </div>
        )}
      </div>
    </header>
