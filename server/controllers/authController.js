...
export async function resendVerificationOTP(req, res) {
  try {
    ...
    const emailUnavailable = /RESEND_API_KEY|email provider|sender|domain/i.test(error?.message || "");

    return res.status(emailUnavailable ? 503 : 500).json({
      success: false,
      message: emailUnavailable
        ? "Email delivery is temporarily unavailable. Please contact FlexHub NG support or try again later."
...
export async function forgotPassword(req, res) {
  try {
    ...
    return res.status(emailUnavailable ? 503 : 500).json({
      success: false,
      message: emailUnavailable
        ? "Password-reset email delivery is temporarily unavailable. Please try again later or contact FlexHub NG support."
