...
function getEmailConfiguration() {
  ...
  const usesResendTestSender = /<onboarding@resend\.dev>$/i.test(sender);
...
export function getEmailServiceStatus() {
  ...
  return {
    ...
    mode: configuration.usesResendTestSender ? "test" : "production",
    warning: configuration.usesResendTestSender
      ? "EMAIL_FROM is using Resend's test sender. Set a sender on a verified domain before emailing arbitrary customers."
      : "",
  };
}
...
export async function verifyEmailTransporter() {
  ...
  if (!status.configured) {
    console.error("❌ RESEND_API_KEY is missing. Verification and password-reset emails are unavailable.");
    return false;
  }

  console.log(`✅ Resend email service is configured with sender: ${status.sender}`);
